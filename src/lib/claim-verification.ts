...
export function verifyClaimAgainst(
  claim: string,
  ranked: RankedChunk[],
  ...
  if (ranked.length === 0 || claimTerms.length === 0) {
    return {
      verdict: "unclear",
      confidence: 0,
      reasons: ranked.length === 0 ? ["no_chunks"] : ["no_significant_claim_terms"],
      supporting_chunks: [],
      refuting_chunks: [],
    };
  }
  ...
  for (const chunk of ranked.slice(0, topN)) {
...
export function combineVerdicts(
  ...
  const supported = perSource.filter((entry) => entry.result.verdict === "supported");
  const refuted = perSource.filter((entry) => entry.result.verdict === "refuted");
  const reasons: string[] = [];
...
function chunkContainsNegationNearTerms(text: string, claimTerms: string[]): boolean {
